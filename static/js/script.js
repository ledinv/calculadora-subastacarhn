...
data.forEach(auto => {
  ...
  // 🔥 FILTRO DE FECHA EN FRONTEND
  if (fechaFiltro) {
    if (auto.fecha_subasta !== fechaFiltro) {
      return;
    }
  }
  ...
  autos.innerHTML += `
    <div class="lote">
      ...
      <div class="col">
        <div class="titulo">
          <a href="${auto.link}" target="_blank" style="text-decoration:none; color:#007bff;">
            ${auto.anio} ${auto.marca} ${auto.modelo}
          </a>
        </div>

        <p><b>Lote:</b> ${auto.lote}</p>
        <p><b>Ubicación:</b> ${auto.ubicacion}</p>
        <p><b>Fecha:</b> ${auto.fecha_subasta}</p>
        <p><b>Destacados:</b> ${auto.destacados}</p>
      </div>

      <div class="col">
        <p><b>Odómetro:</b> ${auto.odometro}</p>
        <p><b>Motor:</b> ${auto.motor}</p>
        <p><b>Transmisión:</b> ${auto.transmision}</p>
        <p><b>Tracción:</b> ${auto.traccion}</p>
        <p><b>Combustible:</b> ${auto.combustible}</p>
        <p><b>Cilindros:</b> ${auto.cilindros}</p>
      </div>

      <div class="col">
        <p><b>Precio:</b> <span style="color:green;font-weight:bold;">${auto.precio}</span></p>
        <p><b>Buy Now:</b> ${auto.buy_now}</p>
        <p><b>Valor:</b> ${auto.valor_mercado}</p>

        <p><b>Daño:</b> ${auto.danio}</p>
        <p><b>Secundario:</b> ${auto.danio_secundario}</p>

        <p><b>Título:</b> ${auto.codigo_titulo}</p>
        <p><b>Llaves:</b> ${auto.llaves}</p>

        <a href="${auto.link}" target="_blank" style="
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  cargarEstados(); // 🔥 AGREGA ESTA LINEA
