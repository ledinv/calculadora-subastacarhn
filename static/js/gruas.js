...
function cargarTiposVehiculo() {
  ...
  vehiculoSelect.innerHTML = '<option value="">Selecciona un tipo de vehículo</option>';

  const tipos = new Set();

  Object.keys(dataGruas).forEach(puerto => {
    Object.keys(dataGruas[puerto]).forEach(ciudadBase => {
      Object.keys(dataGruas[puerto][ciudadBase]).forEach(tipo => {
        tipos.add(tipo);
      });
    });
  });
